...
fun starDistance(firstStar: List<Int>, secondStar: List<Int>, expandingSpace: List<List<Int>>): Int {
    var rowDistance = abs(secondStar[0] - firstStar[0]) + expandingSpace[0].filter { it < max(secondStar[0], firstStar[0]) && it > min(secondStar[0], firstStar[0]) }.size
    var colDistance = abs(secondStar[1] - firstStar[1]) + expandingSpace[1].filter { it < max(secondStar[1], firstStar[1]) && it > min(secondStar[1], firstStar[1]) }.size
...
fun main(args: Array<String>) {
    ...
    println(stars.map { i -> stars.map{ j -> starDistance(i,j,expandingSpace) }.sum() }.sum() / 2)
}